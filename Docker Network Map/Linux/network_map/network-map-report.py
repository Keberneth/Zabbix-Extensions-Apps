...
# Suppress SSL warnings (kept as in original)
...
# Cache for Zabbix history (30 days sliding)
...
# Report time window: last 30 days (in seconds)
DAYS = 30
HISTORY_CHUNK = 24 * 3600  # 1 day chunks
...
# Local cache for NetBox IP->name lookups (per run)
_NETBOX_IP_CACHE = {}
...
def get_all_hosts_ip_map():
    ...
    # Resolve remaining IPs via NetBox only if name equals IP
...
def cleanup_history_cache():
    """
    Remove cached history chunks that are completely older than the current 30-day window.
    """
...
def get_connection_history(itemid):
    """
    Fetch history for given itemid for the last 30 days, using a local cache.

    Data is fetched/stored per DAY (HISTORY_CHUNK) and then filtered
    by TIME_FROM/TIME_TILL when parsing.
    """
    all_entries = []

    # First and last day boundaries covering our interval
    first_chunk_start = (TIME_FROM // HISTORY_CHUNK) * HISTORY_CHUNK
    last_chunk_start = (TIME_TILL // HISTORY_CHUNK) * HISTORY_CHUNK

    # Iterate over all days overlapping the 30-day window
    for chunk_start in range(first_chunk_start, last_chunk_start + 1, HISTORY_CHUNK):
        ...
        # Not cached or cache read failed -> fetch from Zabbix
        ...
        try:
            ...
            print(
                f"[WARN] Failed history.get for item {itemid} from {chunk_start} to {chunk_end}: {e}"
            )
            continue
...
def parse_history_connections(items, ip_to_host):
    ...
    for itm in items:
        ...
        for entry in history:
            ...
            # Keep only data within the 30-day window
...
def separate_overlaps(
    node_positions, node_width=160, node_height=80, padding=40, max_iterations=800
):
...
def main():
    cleanup_history_cache()  # keep cache at 30 days
